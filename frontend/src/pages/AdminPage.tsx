...
type TabType = 'dashboard' | 'permisos' | 'trabajadores' | 'ausencias' | 'usuarios'
...
export default function AdminPage({ user, onLogout }: Props) {
  ...
  const loadData = async () => {
    ...
    try {
      const [statsRes, workersRes, permisosRes, ausenciasRes, directoresRes] = await Promise.all([
        ...
        rrhhService.getAllPermissions({ estado: filterEstado || undefined, tipo: filterTipo || undefined }),
  ...
  useEffect(() => { loadData() }, [filterEstado, filterTipo])
